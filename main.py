...
#Lấy tại BotFather
API_BOT = '8195402269:AAE2hyC5l6Gd1zsHBgmZmaK3qfhFHPKtO-U'
...
admin_ids = [7055636268]  # Thay các ID này bằng ID của admin thực tế
...
def show_main_menu(msg):
    ...
    photo_url = "https://i.imgur.com/F6TbxnY.jpeg"
    caption = """
...
🎗 Tham gia nhóm: https://t.me/minhiosfi5 để nhận Giftcode hàng ngày nhé! 
...
def check_balance(msg):
    ...
    photo_link = "https://i.imgur.com/F6TbxnY.jpeg"
    bot.send_photo(msg.chat.id,
                   photo_link,
                   caption=f"""
...
🏧 Số Tài Khoản: <code>0899256226</code>

🏧Chủ Tài Khoản: <code>HO DUC MINH</code>
...
def set_balance(msg):
  if msg.from_user.id == 6804206350:
    bot.reply_to(msg, """
...
def show_game_options(msg):
    ...
    bot.send_photo(msg.chat.id,
                   photo_link,
                   caption="""
<b> Game Bài Sân Chơi Giải Trí</b>\n
...
def txroom():
    ...
    markup.add(
        telebot.types.InlineKeyboardButton("Game Tài Xỉu",
                                           url="https://t.me/saowingamebaiBOT")),
...
def send_dice_room(group_chat_id2):
    response = requests.get(
        f'https://api.telegram.org/bot8195402269:AAE2hyC5l6Gd1zsHBgmZmaK3qfhFHPKtO-U/sendDice?chat_id=-1002292035515'
    )
    if response.status_code == 200:
        data = response.json()
        if 'result' in data and 'dice' in data['result']:
...
def notify_bet_success(user_id, bet_type, bet_amount):
    ...
    bot.send_message(-1002142338058, bet_message)
...
def send_game_result_and_process_winnings(result, dice_sum, game_result):
    ...
    url_button = types.InlineKeyboardButton(text="Kết Quả TX [ Room ]",
                                            url="https://t.me/ketquatxroomsaowin")
